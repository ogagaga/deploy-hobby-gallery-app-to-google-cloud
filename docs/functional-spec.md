...
### 2.7 堅牢なバリデーションとセキュリティ
- **サーバーサイド検証 (Zod)**: クライアント側だけでなく、Server Actions 内でも `zod` スキーマを用いた厳格なデータ検証を実行。データベース到達前の不正データを完全に遮断。
- **ファイル整合性チェック**: アップロードされた画像のバイナリレベルでのサイズチェック（最大5MB）と、MIME タイプ（image/jpeg, image/png, image/webp）の検証をサーバー側で実施。
- **権限管理の集約**: `checkAdmin` ヘルパーによる認証チェックの共通化により、認可漏れを防止。
