...
Oga's Plastic Model GalleryでGoogle認証を有効にするための手順です。
...
3. プロジェクト名（例: `Oga's Plastic Model Gallery`）を入力し、「作成」をクリックします。
...
3. **アプリ情報** を入力します（以下の項目以外は未入力でOK）。
   - **アプリ名**: `Oga's Plastic Model Gallery`
...
4. **名前** に `Oga's Plastic Model Gallery (Local)` などと入力します。
